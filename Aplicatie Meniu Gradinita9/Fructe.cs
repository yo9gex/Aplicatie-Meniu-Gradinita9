...
{
    public partial class Fructe : UserControl
    {
        ...
        public void DisplayFructe()
        {
            ...
            List<FructeData> produseList = fd.produseFructeListData();
            dataFructe.DataSource = produseList;
        }
        ...
        private void sterge_btn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (conn.State == ConnectionState.Closed)
                    ...
                    {
                        conn.Open();
                        string query = "DELETE FROM fructe WHERE nume=@NumeProdus";
            ...
            else
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        string query = "UPDATE fructe SET proteine=@Proteine, lipide=@Lipide, glucide=@Glucide, calorii=@Calorii, scazamant=@Scazamant, status=@Status, " +
            ...
            else
            {
                if (conn.State == ConnectionState.Closed)
                    try
                    {
                        ...
                        string checkNume = "SELECT COUNT(*) FROM fructe WHERE nume = @nume";
                        ...
                        string query = "INSERT INTO fructe (nume, proteine, proteine_vegetale, proteine_animale, lipide, lipide_vegetale, lipide_animale, glucide, fier, calciu, calorii, grupa_produse1, grupa_produse2, coeficient_echivalent1, coeficient_echivalent2, scazamant, cantitate, status) " +
        ...
        private void dataFructe_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1)
            {
                DataGridViewRow row = dataFructe.Rows[e.RowIndex];
