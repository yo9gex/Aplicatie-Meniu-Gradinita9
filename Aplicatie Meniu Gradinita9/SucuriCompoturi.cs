...
{
    public partial class SucuriCompoturi : UserControl
    {
        ...
        {
            SucuriData sc = new SucuriData();
            List<SucuriData> produseList = sc.produseSucuriListData();
            dataSucuriCompoturi.DataSource = produseList;
        }
        ...
        private void sterge_btn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (conn.State == ConnectionState.Closed)
                    ...
                    {
                        conn.Open();
                        string query = "DELETE FROM sucuriCompoturi WHERE nume=@NumeProdus";
            ...
            else
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        string query = "UPDATE sucuriCompoturi SET proteine=@Proteine, lipide=@Lipide, glucide=@Glucide, calorii=@Calorii, scazamant=@Scazamant, status=@Status, " +
            ...
            else
            {
                if (conn.State == ConnectionState.Closed)
                    try
                    {
                        ...
                        string checkNume = "SELECT COUNT(*) FROM sucuriCompoturi WHERE nume = @nume";
                        ...
                        string query = "INSERT INTO sucuriCompoturi (nume, proteine, proteine_vegetale, proteine_animale, lipide, lipide_vegetale, lipide_animale, glucide, fier, calciu, calorii, grupa_produse1, grupa_produse2, coeficient_echivalent1, coeficient_echivalent2, scazamant, cantitate, status) " +
        ...
        private void dataSucuriCompoturi_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1)
            {
                DataGridViewRow row = dataSucuriCompoturi.Rows[e.RowIndex];
