...
namespace Aplicatie_Meniu_Gradinita9
{
...

    {
        ...
        // reimprospateaza datele in data grid view
        ...
        // afiseaza produsele derivate din cereale in data grid view
        public void displayProduseDerivate()
        {
            
            ProduseDerivateData pd = new ProduseDerivateData();
            List<ProduseDerivateData> produseList = pd.produseDerivateListData();
            dataProduseDerivateCereale.DataSource = produseList;
            ...
            else
            {
                if (conn.State == ConnectionState.Closed)
                    try
                    {
                        ...
                        string checkNume = "SELECT COUNT(*) FROM produseDerivateCereale WHERE nume = @nume";
                        ...
                        string query = "INSERT INTO produseDerivateCereale (nume, proteine, proteine_vegetale, proteine_animale, lipide, lipide_vegetale, lipide_animale, glucide, fier, calciu, calorii, grupa_produse1, grupa_produse2, coeficient_echivalent1, coeficient_echivalent2, scazamant, cantitate, status) " +
        ...
        private void sterge_btn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (conn.State == ConnectionState.Closed)
                    ...
                    {
                        conn.Open();
                        string query = "DELETE FROM produseDerivateCereale WHERE nume=@NumeProdus";
        ...
        private void clear_btn_Click(object sender, EventArgs e)
        {
            ...
            //decimal[] cant = new decimal[] { 30M, 25M, 0.1M, 30M, 0M, 0M };
        }
        ...
        // afiseaza datele in campuri cand se da click pe un rand din data grid view
        private void dataProduseDerivateCereale_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if(e.RowIndex != -1)
            {
                DataGridViewRow row = dataProduseDerivateCereale.Rows[e.RowIndex];
        ...
        // actualizeaza produs in baza de date
