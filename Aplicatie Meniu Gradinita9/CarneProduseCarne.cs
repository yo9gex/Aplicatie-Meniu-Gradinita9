...
{
    public partial class CarneProduseCarne : UserControl
    {
       // SqlConnection conn = new SqlConnection(@"Data Source=(localdb)\MSSQLLocalDB;AttachDbFilename=C:\Users\LeVantinik\Documents\meniul.mdf;Integrated Security=True;Connect Timeout=30");
        ...
        public void RefreshData()
        {
            ...
            DisplayCarneProduseCarne();
        }
        public CarneProduseCarne()
        {
            InitializeComponent();
        ...
        public void DisplayCarneProduseCarne()
        {
            CarneData cd = new CarneData();
            List<CarneData> produseList = cd.produseCarneListData();
            dataCarneProduseCarne.DataSource = produseList;
        }
        ...
        private void sterge_btn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (conn.State == ConnectionState.Closed)
                    ...
                    {
                        conn.Open();
                        string query = "DELETE FROM carneProduseCarne WHERE nume=@NumeProdus";
            ...
            else
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        string query = "UPDATE carneProduseCarne SET proteine=@Proteine, lipide=@Lipide, glucide=@Glucide, calorii=@Calorii, scazamant=@Scazamant, status=@Status, " +
            ...
            else
            {
                if (conn.State == ConnectionState.Closed)
                    try
                    {
                        ...
                        string checkNume = "SELECT COUNT(*) FROM carneProduseCarne WHERE nume = @nume";
                        ...
                        string query = "INSERT INTO carneProduseCarne (nume, proteine, proteine_vegetale, proteine_animale, lipide, lipide_vegetale, lipide_animale, glucide, fier, calciu, calorii, grupa_produse1, grupa_produse2, coeficient_echivalent1, coeficient_echivalent2, scazamant, cantitate, status) " +
        ...
        private void dataCarneProduseCarne_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1)
            {
                DataGridViewRow row = dataCarneProduseCarne.Rows[e.RowIndex];
