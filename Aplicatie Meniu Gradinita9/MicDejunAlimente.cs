...
{
    public partial class MicDejunAlimente : UserControl
    {
        public string Date;
        ...
        public void RefreshData()
        {
            ...
            CalculeParametriNutrienti(out decimal totalProteine, out decimal totalLipide, out decimal totalGlucide, out decimal totalCalorii);
            DisplayMicDejunAlimenteActualizate();


        }
        public void DisplayMicDejunAlimente()
        {
            MicDejunAlimenteData mdad = new MicDejunAlimenteData();
            List<MicDejunAlimenteData> mdList = mdad.ListaMicDejunAlimeteData();
            dataMicDejunAlimente.DataSource = mdList;
        }
        public void DisplayMicDejunAlimenteActualizate()
        {
            MicDejunAlimenteDataActualizate mdada = new MicDejunAlimenteDataActualizate();
            List<MicDejunAlimenteDataActualizate> mdadaList = mdada.ListaMicDejunAlimeteDataActualizate();
            dataMicDejunAlimente.DataSource = mdadaList;

        }
        private void MicDejunAlimente_Load(object sender, EventArgs e)
        {
            TabelaMicDejun();
            // StergeProdusDinMicDejun();
            dataToday_lbl.Text = Date;
            DisplayMicDejunAlimenteActualizate();

            meniuMicDejun_comboBox.DataSource = ListaMeniuri.MicDejun;
            meniuMicDejun_comboBox.SelectedIndexChanged += meniuMicDejun_comboBox_SelectedIndexChanged;
        }

        private void meniuMicDejun_comboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (meniuMicDejun_comboBox.SelectedItem != null)
            {
                numeMeniu_txt.Text = meniuMicDejun_comboBox.SelectedItem.ToString();
                if (MeniulZileiInstance != null)
                {
                    MeniulZileiInstance.MicDejunText = meniuMicDejun_comboBox.SelectedItem.ToString();
                }
            }
        }
        ...
        private void dataMicDejunAlimente_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1)
            {
                DataGridViewRow row = this.dataMicDejunAlimente.Rows[e.RowIndex];
        ...
        public void TabelaMicDejun()
        {
           ...
           // string connectionString = @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=""F:\8_Proiecte visual studio\Test\Aplicatie Meniu Gradinita9\Aplicatie Meniu Gradinita9\meniul.mdf"";Integrated Security=True";
            ...
            string queryy = @"INSERT INTO totalAlimenteMicDejun  (nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status)
                              ...
                              FROM produseDerivateCereale p WHERE status = 'Alege' AND (tip_meniu = 'MD' OR tip_meniu = 'MD+P' OR tip_meniu = 'MD+G')
                             AND NOT EXISTS ( SELECT 1 FROM totalAlimenteMicDejun t WHERE t.nume = p.nume);
                              ";
            string queryy2 = @"INSERT INTO totalAlimenteMicDejun  (nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status)
                              SELECT nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status 
                              FROM laptePreparateLapte p WHERE status = 'Alege' AND (tip_meniu = 'MD' OR tip_meniu = 'MD+P' OR tip_meniu = 'MD+G')
                             AND NOT EXISTS ( SELECT 1 FROM totalAlimenteMicDejun t WHERE t.nume = p.nume);
                              ";
            string queryy3 = @"INSERT INTO totalAlimenteMicDejun  (nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status)
                              SELECT nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status 
                              FROM carneProduseCarne p WHERE status = 'Alege' AND (tip_meniu = 'MD' OR tip_meniu = 'MD+P' OR tip_meniu = 'MD+G')
                             AND NOT EXISTS ( SELECT 1 FROM totalAlimenteMicDejun t WHERE t.nume = p.nume);
                              ";
            ...
            string queryy5 = @"INSERT INTO totalAlimenteMicDejun  (nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status)
                              SELECT nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status 
                              FROM fructe p WHERE status = 'Alege' AND (tip_meniu = 'MD' OR tip_meniu = 'MD+P' OR tip_meniu = 'MD+G')
                             AND NOT EXISTS ( SELECT 1 FROM totalAlimenteMicDejun t WHERE t.nume = p.nume);
                              ";
            string queryy6 = @"INSERT INTO totalAlimenteMicDejun  (nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status)
                              SELECT nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status 
                              FROM fructeOleaginoase p WHERE status = 'Alege' AND (tip_meniu = 'MD' OR tip_meniu = 'MD+P' OR tip_meniu = 'MD+G')
                             AND NOT EXISTS ( SELECT 1 FROM totalAlimenteMicDejun t WHERE t.nume = p.nume);
                              ";
            string queryy7 = @"INSERT INTO totalAlimenteMicDejun  (nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status)
                              SELECT nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status 
                              FROM legumeConservate p WHERE status = 'Alege' AND (tip_meniu = 'MD' OR tip_meniu = 'MD+P' OR tip_meniu = 'MD+G')
                             AND NOT EXISTS ( SELECT 1 FROM totalAlimenteMicDejun t WHERE t.nume = p.nume);
                              ";
            string queryy8 = @"INSERT INTO totalAlimenteMicDejun  (nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status)
                              SELECT nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status 
                              FROM legumeProaspete p WHERE status = 'Alege' AND (tip_meniu = 'MD' OR tip_meniu = 'MD+P' OR tip_meniu = 'MD+G')
                             AND NOT EXISTS ( SELECT 1 FROM totalAlimenteMicDejun t WHERE t.nume = p.nume);
                              ";
            string queryy9 = @"INSERT INTO totalAlimenteMicDejun  (nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status)
                              SELECT nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status 
                              FROM oua p WHERE status = 'Alege' AND (tip_meniu = 'MD' OR tip_meniu = 'MD+P' OR tip_meniu = 'MD+G')
                             AND NOT EXISTS ( SELECT 1 FROM totalAlimenteMicDejun t WHERE t.nume = p.nume);
                              ";
            string queryy10 = @"INSERT INTO totalAlimenteMicDejun  (nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status)
                              SELECT nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status 
                              FROM produseZaharoase p WHERE status = 'Alege' AND (tip_meniu = 'MD' OR tip_meniu = 'MD+P' OR tip_meniu = 'MD+G')
                             AND NOT EXISTS ( SELECT 1 FROM totalAlimenteMicDejun t WHERE t.nume = p.nume);
                              ";
            string queryy11 = @"INSERT INTO totalAlimenteMicDejun  (nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status)
                              SELECT nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status 
                              FROM sucuriCompoturi p WHERE status = 'Alege' AND (tip_meniu = 'MD' OR tip_meniu = 'MD+P' OR tip_meniu = 'MD+G')
                             AND NOT EXISTS ( SELECT 1 FROM totalAlimenteMicDejun t WHERE t.nume = p.nume);
                              ";
            string queryy12 = @"INSERT INTO totalAlimenteMicDejun  (nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status)
                              SELECT nume, proteine, lipide, glucide, calorii, scazamant, cantitate, TProteine, TLipide, TGlucide, TCalorii, TCantitate, TNet, tip_meniu, status 
                              FROM ulei p WHERE status = 'Alege' AND (tip_meniu = 'MD' OR tip_meniu = 'MD+P' OR tip_meniu = 'MD+G')
                             AND NOT EXISTS ( SELECT 1 FROM totalAlimenteMicDejun t WHERE t.nume = p.nume);
                              ";
        ...
        public void StergeProdusDinMicDejun()
        {
           // string connectionString = @"Data Source=(localdb)\MSSQLLocalDB;AttachDbFilename=C:\Users\LeVantinik\Documents\meniul.mdf;Integrated Security=True;Connect Timeout=30";
           // string connectionString = @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=""F:\8_Proiecte visual studio\Test\Aplicatie Meniu Gradinita9\Aplicatie Meniu Gradinita9\meniul.mdf"";Integrated Security=True";
            ...
            else
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    if (connection.State == ConnectionState.Closed)
                    {
                        ...
                        {
                            connection.Open();
                            string updateData = "UPDATE totalAlimenteMicDejun SET cantitate=@cantitate WHERE nume = @nume ";
        ...
        public void CalculeParametriNutrienti(out decimal totalProteine, out decimal totalLipide, out decimal totalGlucide, out decimal totalCalorii)
        {
            totalProteine = 0;
            totalLipide = 0;
            totalGlucide = 0;
           ...
           // string connectionString = @"Data Source=(localdb)\MSSQLLocalDB;AttachDbFilename=C:\Users\LeVantinik\Documents\meniul.mdf;Integrated Security=True;Connect Timeout=30";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    connection.Open();
                    string query = "Select * FROM totalAlimenteMicDejun WHERE status = 'Alege' ";
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        ...
                        foreach (var produs in produse)
                        {
                            ...
                            string updateQuery = "UPDATE totalAlimenteMicDejun SET TCantitate = @cantitateTotala, TNet = @totalNet, TProteine = @proteineTotale, TLipide = @lipideTotale, TGlucide = @glucideTotale, TCalorii = @caloriiTotale WHERE status = 'Alege'  AND nume = @nume";
            ...
            for (int i = 0; i < dataMicDejunAlimente.Rows.Count; i++)
            {

                if (dataMicDejunAlimente.Rows[i].Cells["TProteine"].Value != DBNull.Value &&
                    dataMicDejunAlimente.Rows[i].Cells["TLipide"].Value != DBNull.Value &&
                    dataMicDejunAlimente.Rows[i].Cells["TGlucide"].Value != DBNull.Value &&
                    dataMicDejunAlimente.Rows[i].Cells["TCalorii"].Value != DBNull.Value)
                {
                    totalProteine += Convert.ToDecimal(dataMicDejunAlimente.Rows[i].Cells["TProteine"].Value);
                    totalLipide += Convert.ToDecimal(dataMicDejunAlimente.Rows[i].Cells["TLipide"].Value);
                    totalGlucide += Convert.ToDecimal(dataMicDejunAlimente.Rows[i].Cells["TGlucide"].Value);
                    totalCalorii += Convert.ToDecimal(dataMicDejunAlimente.Rows[i].Cells["TCalorii"].Value);
