...
{
    public partial class ProduseZaharoase : UserControl
    {
        ...
        public void displayProduseZaharoase()
        {
            ...
            List<ZaharoaseData> produseList = zd.produseZaharoaseListData();
            dataProduseZaharoase.DataSource = produseList;
        ...
        private void sterge_btn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (conn.State == ConnectionState.Closed)
                    ...
                    {
                        conn.Open();
                        string query = "DELETE FROM produseZaharoase WHERE nume=@NumeProdus";
            ...
            else
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        string query = "UPDATE produseZaharoase SET proteine=@Proteine, lipide=@Lipide, glucide=@Glucide, calorii=@Calorii, scazamant=@Scazamant, status=@Status, " +
            ...
            else
            {
                if (conn.State == ConnectionState.Closed)
                    try
                    {
                        ...
                        string checkNume = "SELECT COUNT(*) FROM produseZaharoase WHERE nume = @nume";
                        ...
                        string query = "INSERT INTO produseZaharoase (nume, proteine, proteine_vegetale, proteine_animale, lipide, lipide_vegetale, lipide_animale, glucide, fier, calciu, calorii, grupa_produse1, grupa_produse2, coeficient_echivalent1, coeficient_echivalent2, scazamant, cantitate, status) " +
        ...
        private void dataProduseZaharoase_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1)
            {
                DataGridViewRow row = dataProduseZaharoase.Rows[e.RowIndex];
